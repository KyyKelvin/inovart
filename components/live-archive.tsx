  ...
  useEffect(() => {
    ...
    client.from(kind).select(selection).eq("status", "published").order("featured", { ascending: false }).order("published_at", { ascending: false }).limit(featured ? 3 : 100).then(({ data, error }) => {
      ...
      const mapped = kind === "artisans"
        ? (data as unknown as Artisan[]).map(x => ({ slug: x.slug, title: x.name, subtitle: x.craft, place: x.region_withheld ? "Região não informada" : x.neighborhood || "Varginha · MG", image: safeMediaUrl(x.portrait_url), categories: x.artisan_categories?.flatMap(c => c.categories ? [{ slug: c.categories.slug, name: c.categories.name }] : []) || [] }))
