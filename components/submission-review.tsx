...
export function SubmissionReview({ id }: { id: string }) {
  ...
  return <div><Link className="text-link" href="/admin/submissoes">← Voltar à fila</Link><p className="status" role="status">{message}</p>{entry && <>
    <div className="profile-layout"><div><span className="chip">{entry.status}</span><h1>{entry.name}</h1><p className="eyebrow">{entry.craft}</p><p className="preserve-lines">{entry.bio}</p><dl><dt>Região</dt><dd>{entry.region_withheld ? "Não informada por escolha do participante" : entry.neighborhood || "Não informada"}</dd><dt>E-mail</dt><dd>{entry.email}</dd><dt>Telefone</dt><dd>{entry.phone || "Não informado"}</dd><dt>Instagram</dt><dd>{entry.instagram || "Não informado"}</dd></dl></div>{entry.portrait_path && images[entry.portrait_path] && <img className="profile-image" src={images[entry.portrait_path]} alt="Retrato enviado para avaliação" decoding="async" />}</div>
    <div className="admin-list">{entry.submission_works.map(work => <article key={work.id} className="window form-padding"><h2>{work.title}</h2><p className="preserve-lines">{work.description}</p><dl className="submission-work-meta"><div><dt>Materiais</dt><dd>{work.materials.join(" · ") || "Não informados"}</dd></div><div><dt>Preço</dt><dd>{formatBrlFromCents(work.price_cents)}</dd></div><div><dt>Envio / retirada</dt><dd>{work.shipping_details || "A combinar"}</dd></div></dl><div className="media-gallery">{work.image_paths.filter(p => images[p]).map((path, index) => <img key={path} src={images[path]} alt={`${work.title}, imagem ${index + 1}`} loading="lazy" decoding="async" />)}</div></article>)}</div>
